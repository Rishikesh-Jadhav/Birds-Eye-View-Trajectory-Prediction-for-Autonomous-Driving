...
class IntersectionOverUnion(Metric):
    ...
    def __init__(
        ...
        self.metric = MulticlassStatScores(num_classes=self.n_classes, average='micro')
    ...
    #     self.true_positive += scores[0].item()
    #     self.false_positive += scores[1].item()
    #     self.false_negative += scores[3].item()
    #     self.support += scores[4].item()

    def update(self, prediction: torch.Tensor, target: torch.Tensor):
        scores = self.metric(prediction, target)
        # tps, fps, _, fns, sups = MulticlassStatScores(prediction, target, self.n_classes)
