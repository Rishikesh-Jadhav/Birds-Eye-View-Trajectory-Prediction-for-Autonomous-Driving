...
def convert_figure_numpy(figure):
    # Use FigureCanvasAgg instead of FigureCanvasTkAgg
    canvas = FigureCanvasAgg(figure)
    canvas.draw()

    # Convert the figure to a NumPy array
    figure_np = np.frombuffer(canvas.tostring_rgb(), dtype=np.uint8)
    figure_np = figure_np.reshape(canvas.get_width_height()[::-1] + (3,))
